...
async function signIn(){
  ...
  try{
    ...
    if(e && e.code === 'auth/unauthorized-domain'){
      alert('Doména není povolená v Firebase Auth. Přidej Authorized domain.');
    } else {
...
window.signIn = signIn; window.signOutUser = signOutUser; window.debugSignIn = signIn;

// onAuthStateChanged: toggle admin UI
const btnSignIn = document.getElementById('btn-signin');
const btnSignOut = document.getElementById('btn-signout');
const userEmailSpan = document.getElementById('user-email');
const adminForms = document.getElementById('admin-forms');
const notAdmin = document.getElementById('not-admin');
const adminRaceList = document.getElementById('admin-race-list');
...
function calculateLeaderboardForSeason(races, year){
  const filtered = races.filter(r=> r.date && new Date(r.date).getFullYear()===year);
  return calculateLeaderboardFromRaces(filtered);
}

function calculateAllTimeStats(races){
  const stats = {};
  races.forEach(race=>{
    if(!race.results) return;
    for(let i=1;i<=maxPositions;i++){
      ...
      const d = res.driver;
      const pts = res.dns?0:(pointsForPosition[i]||0);
      if(!stats[d]) stats[d] = { driver:d, points:0, wins:0, starts:0 };
      stats[d].points += pts; stats[d].starts++; if(i===1) stats[d].wins++;
    }
  });
  return Object.values(stats).sort((a,b)=>b.points - a.points);
}
...
// Chart.js usage (global Chart)
let mainChart = null;
function renderPointsChart(leaderboard, totalRaces){
  const ctx = document.getElementById('points-chart').getContext('2d');
  const datasets = leaderboard.map(d=>{
    const data = [];
    for(let i=0;i<totalRaces;i++){ const entry = d.pointsHistory.find(p=>p.raceIndex===i); data.push(entry?entry.cumulative:(data[i-1]||0)); }
...
function renderArchive(races){
  ...
  const byYear = {}; races.forEach(r=>{ const y = new Date(r.date).getFullYear(); if(!byYear[y]) byYear[y]=[]; byYear[y].push(r); });
...
// Admin helpers: populate selects and position inputs
...
function editRace(id){
  ...
  document.getElementById('additional-info').value = r.additionalInfo||'';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  raceForm.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      resetRaceForm();
    }catch(err){ alert('Chyba při ukládání závodu: '+(err.message||err)); }
  });
  ...
  resultsForm.addEventListener('submit', async (e)=>{
    ...
    if(!raceId){ alert('Vyberte závod'); return; }
    const results = {};
    for(let i=1;i<=maxPositions;i++){
      results['pos'+i] = {
        driver: document.getElementById(`pos${i}`).value,
        manufacturer: document.getElementById(`pos${i}_manufacturer`).value,
        model: document.getElementById(`pos${i}_model`).value,
        drivetrain: document.getElementById(`pos${i}_drivetrain`).value,
        dns: document.getElementById(`pos${i}_dns`).checked
      };
    }
    const youtube = document.getElementById('youtube-url').value;
    try{
      await updateRaceInFirestore(raceId, { results, youtubeUrl: youtube||null });
      resetResultsForm();
    }catch(err){ alert('Chyba při ukládání výsledků: '+(err.message||err)); }
  });
